<html>    
	...
	<?php 
        ...
        <div id="container">
			...
			$id = $_GET["id"]; 
//try retrieving data from violations where violation_id = $id		
			try {
		...
		if ($data = $stmt->fetch()){
			...
			//open pdo to database
				$conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
				$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			//that selects appropriate fields from violations table where violation_id = $id, status = unpaid and type = smoking	
				$stmt = $conn->prepare("SELECT violation_id, date, time, v_firstname, v_surname, description, v_department, v_supervisor, place FROM violations WHERE violation_id='$id' AND status = 'Unpaid' AND violation_type = 'Smoking'"); 
				$stmt->execute();
