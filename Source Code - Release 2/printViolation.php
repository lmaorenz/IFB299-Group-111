<html>    
	...
	<?php 
        ...
        <div id="container">
			...
			$_SESSION['id']=$_GET["id"];
			?>
			<h1>Parking Violation Details</h1>
			<br>
			<?php
...
//try retirieving data from violations where violation_id = $id		
			try {
		...
		if ($data = $stmt->fetch()){
			...
			//open pdo to database
				$conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
				$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			//that selects appropriate fields from violations table where violation_id = $id, status = unpaid and type = parking
				$stmt = $conn->prepare("SELECT violation_id, date, time, v_firstname, v_surname, description, vehicle_type, license FROM violations WHERE violation_id='$id' AND status = 'Unpaid' AND violation_type = 'Parking'"); 
				$stmt->execute();
