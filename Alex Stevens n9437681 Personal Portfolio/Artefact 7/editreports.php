...
<html>    
	...
	<?php 
		...
		<div id='captionHead'>H&S Report Database</div>
        <div id="container">
            <br>
			<?php
			session_start();
		...
		if ($data = $stmt->fetch()){
			...
			<form name="edit" action="editreports.php" method="post" onSubmit="return checkTime()">
                    <h2>Update Health and Safety Report</h2>
					<h2>Name</h2>
					...
					<h2>Description</h2>
                        <textarea row="8" cols="50" name="desc" value="<?php echo $row->description; ?>"></textarea>
					<br>
					...
					<h2>Resolution</h2>
                        <textarea row="8" cols="50" name="res" value="<?php echo $row->resolution; ?>"></textarea>
                    <h2>Status</h2>
